...
These are asserted by the shared contract suite. A component that skips one
fails CI.
...
**4. `Modifier.minimumTouchTarget()` on anything interactive.** Before any
`size` modifier, so it wraps the visual size rather than being overridden by it.

**5. Tokens only.** No `Color(0xFF…)`, no `12.dp` radius, no `220` duration.
...
- **Registration in the contract suite**, which covers the five rules above.
- **Behaviour tests** for anything the contract suite cannot know: that a
  disabled button does not fire `onClick`, that a slider clamps, that a text
  field's output transformation masks correctly.
