...
### What LineageOS actually carries, measured rather than assumed

Two sources, both raw files, no clones and no GitHub API — which is just as
well, since the API is blocked from some sandboxes and `raw.githubusercontent.com`
is not:
...
def pull() -> dict:
    ...
    print(
        ...
        f"{len(measured)} declaring a corner radius"
    )
    return {
        "source": "LineageOS/hudson and LineageOS/lineage_wiki, via raw.githubusercontent.com",
...
def main() -> int:
    ...
    if not TABLE.exists() or TABLE.read_text() != wanted:
        ...
        return 1
    print(f"device corner table: {len(seen)} devices, {len(table)} bytes packed, in step")
    return 0
