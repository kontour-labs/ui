...
| [`ExpandingListItem`](#expandinglistitem) | A row that unfolds more rows | `Accordion`, when what opens is *content* |
...
a [`ListGroup`](#listitem) does, so what comes out is rows rather than a panel.
...
**Reach for [`Accordion`](display.md#accordion) instead** when what opens is
