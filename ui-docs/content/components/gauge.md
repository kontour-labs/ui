...
- **One colour, a gradient, or bands** — `GaugeDefaults.colours(indicator = …)`
  ...
  `bands(start) { band(from = …, colour = …) }`. Whichever it is, the colours run
...
band — with `ScaleColours.bands`. The first colour runs from the start of the
scale, and each `band(from = …)` from its value until the next one begins. **The
values are in the gauge's own units** — revolutions, degrees — and the gauge maps
them onto its `valueRange` when it draws, so the scale is said once, on the gauge,
and the bands never repeat it; the last band runs to wherever the scale ends. They
...
of the way to its neighbours. A blend never reaches past halfway into either band,
so a narrow one keeps its colour at its middle however smooth the rest.
